...
import { EmailDeliveryUnavailableError, sendPasswordResetEmail } from "../lib/email";
...
test("production fails closed when SMTP is disabled", async () => {
  const snapshot = snapshotEnv();
  try {
    ...
    await assert.rejects(
      () =>
        sendPasswordResetEmail({
          email: "user@example.com",
          name: "Test User",
          resetUrl: "https://your-app.example.com/auth/reset-password?token=abc",
        }),
...
test("production never logs the reset URL when SMTP is misconfigured", async () => {
  const snapshot = snapshotEnv();
  ...
  try {
    ...
    await sendPasswordResetEmail({
      email: "user@example.com",
      name: "Test User",
      resetUrl: "https://your-app.example.com/auth/reset-password?token=SECRET-TOKEN",
    }).catch(() => undefined);
    ...
    for (const message of capturedLogs) {
      assert.equal(
        ...
        false,
        `Production logs must not include the raw token: ${message}`,
      );
    }
  } finally {
